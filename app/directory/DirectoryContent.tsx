...
export default function DirectoryContent() {
  ...
  const loadCommunityInsights = async () => {
    ...
    const grouped = ratings.reduce((acc: any, rating: any) => {
      if (!acc[rating.listing_id]) {
        acc[rating.listing_id] = []
      }
    ...
    Object.keys(grouped).forEach((listingId) => {
      ...
      insightsMap.set(parseInt(listingId), {
        ...
        rating_count: count,
        taste_style: mostCommonTaste,
        avg_price: avgPrice,
        avg_portion_size: avgPortion,
        parking: mostCommonParking,
      })
    })
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {!loading && filteredListings.length === 0 && (
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            <p className="text-2xl text-gray-600">No restaurants found</p>
          </div>
        )}
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-purple-800 space-y-0.5">
                              {insight.taste_style && (
                                <p>🇵🇭 {getTasteLabel(insight.taste_style)}</p>
                              )}
                              {insight.avg_price && (
                                <p>💵 {getPriceLabel(insight.avg_price)}</p>
                              )}
                            </div>
                          </div>
                        </div>
