...
export async function POST(req: NextRequest) {
  try {
    ...
    // Notify admin
    const brevoKeyDefined = !!process.env.BREVO_API_KEY
    console.log('[claim-listing] BREVO_API_KEY defined:', brevoKeyDefined)
    ...
    try {
      ...
      const adminEmailBody = await adminEmailRes.json().catch(() => ({}))
      console.log('[claim-listing] admin notification email status:', adminEmailRes.status, JSON.stringify(adminEmailBody))
    } catch (emailErr) {
      console.error('[claim-listing] admin notification email failed:', emailErr)
    }

    // Confirmation to claimant
    try {
      const claimantEmailRes = await fetch('https://api.brevo.com/v3/smtp/email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'api-key': process.env.BREVO_API_KEY!,
      ...
      const claimantEmailBody = await claimantEmailRes.json().catch(() => ({}))
      console.log('[claim-listing] claimant confirmation email status:', claimantEmailRes.status, JSON.stringify(claimantEmailBody))
    } catch (emailErr) {
      console.error('[claim-listing] claimant confirmation email failed:', emailErr)
    }
