...
// Create LatestNews component
const component = `import { createClient } from '@supabase/supabase-js'
...
fs.writeFileSync('app/components/LatestNews.tsx', component);
console.log('Component created. Size:', fs.statSync('app/components/LatestNews.tsx').size, 'bytes');
...
// Add component after FeaturedArticles
page = page.replace(
  '<FeaturedArticles />',
  '<FeaturedArticles />\n      <LatestNews />'
);
...
console.log('Homepage updated. Verify:');
const lines = page.split('\n');
lines.forEach((l,i)=>{ if(l.includes('LatestNews')) console.log(i+1, l.trim()); });
